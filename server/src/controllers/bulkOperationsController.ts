    ...
    switch (action) {
      ...
      case 'update-status':
        ...
        if (data.status === 'won' || data.status === 'lost') {
          const oppStage = data.status === 'won' ? 'closed_won' : 'closed_lost';
          try {
            ...
            console.error('Failed to auto-update opportunities from bulk lead update:', err);
          }
        }
      ...
      case 'delete':
        if (!isAdmin(user)) {
          return ResponseHandler.forbidden(res, 'Only Admins can perform bulk deletion');
        }

        // Check for attached opportunities in closed stages
        const opportunitiesBulk = await prisma.opportunity.findMany({
            where: { leadId: { in: leadIds }, isDeleted: false }
        });

        const openOpportunitiesBulk = opportunitiesBulk.filter(opp => opp.stage !== 'closed_won' && opp.stage !== 'closed_lost');

        const transactionOperationsBulk = [];

        if (openOpportunitiesBulk.length > 0) {
            transactionOperationsBulk.push(
                prisma.opportunity.updateMany({
                    where: { id: { in: openOpportunitiesBulk.map(o => o.id) } },
                    data: { isDeleted: true }
                })
            );
        }
        ...
        await prisma.$transaction(transactionOperationsBulk);
        
        // Mock result count for legacy code compatibility
